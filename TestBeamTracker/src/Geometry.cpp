...
std::vector<uint> Geometry::planeNumbers = {0,1}; 
    ...
    */
    std::vector<Acts::Vector3D> Geometry::planePos = {
        ...
        {1200.98847384979979, -1.3868291665321006, 0.20013615629889792},
        {900.14576064120047, 0.96974371713907281, -1.2780269284105472},
        //{650.86442031662079, -1.0298738572108768, -0.29759448154141843},
        //{529.51825382438506, 0.32012103785691259, -0.24600216250640017},
        //{230.53340996208263, -1.2782066076189480, 0.76577027338912718},
        //{120.50613379500690, 0.66139840704955066, -1.7351962693193332},
        //{0.0000000000000000, 0.0000000000000000, 0.0000000000000000},
        };
...
Geometry::Geometry() {
  ...
  for (uint iPlane=0; iPlane<nPlanes; ++iPlane) {
    ...
    //cfg.rBounds = std::make_shared<const RectangleBounds>(0.5*width(), 0.5*height());
    cfg.rBounds = std::make_shared<const RectangleBounds>(RectangleBounds(0.5*width(),0.5*height()));
  ...
  std::cout<<"Setting up the tracking Geometry builder...."<<std::endl;
  ...
  ///OLD
  //tgbCfg.trackingVolumeBuilders.push_back(std::make_shared<const CuboidVolumeBuilder>(cvb));

  
  //test1 and test2 aren't used, but I'm passing them for making it compile.
  tgbCfg.trackingVolumeBuilders.push_back(
      [=](const auto& dummy_context, const auto& test1, const auto& test2) 
      { return  cvb.trackingVolume(dummy_context,test1,test2);}
      );
...
// (must be static to be callable by `Acts::TrackingGeometry::visitSurfaces`)
    ...
    //uint Geometry::planeNumber(const Acts::Surface & surf, const Acts::GeometryContext* context) {

  // Allow wiggle room from center position due to rotation.
    //double eps = 30.0 * Acts::units::_mm;

    //Acts::Vector3D center = surf.center(context);
